...
use tokio_postgres::{
    ...
    replication::{LogicalReplicationStream, ReplicationStream},
    types::{PgLsn, Type},
...
pub enum ReplicationClientError {
    ...
    #[error("tpye oid column is not a valid i32")]
    TypeOidColumnNotI32,
...
impl ReplicationClient {
    ...
    pub async fn get_table_schemas(
        ...
        for table_name in table_names {
            let table_schema = self.get_table_schema_by_name(table_name.clone()).await?;
            ...
            .await?
        {
            if let SimpleQueryMessage::Row(row) = message {
                ...
                let typ = row
                    ...
                    .parse::<i32>()
                    .map_err(|_| ReplicationClientError::TypeOidColumnNotI32)?;

                column_schemas.push(ColumnSchema { name, type_id: typ })
            }
        }
    ...
    pub async fn get_logical_replication_stream(
        ...
        info!("starting logical replication stream with query: {}", query);
        ...
        info!("successfully started logical replication stream");
